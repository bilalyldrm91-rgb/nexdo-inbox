...
export default async function handler(req, res) {
  ...
  for(const channel of channels) {
    ...
    try {
      const emails = await fetchEmails(email, password, host, port, sinceStr);
      let saved = 0;
      ...
      totalSaved += saved;
      results.push({ workspace_id: channel.workspace_id, email, host, saved, total: emails.length });
...
function fetchEmails(user, password, host, port, since) {
